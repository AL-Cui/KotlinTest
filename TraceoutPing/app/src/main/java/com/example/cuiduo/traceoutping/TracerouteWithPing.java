package com.example.cuiduo.traceoutping;

/**
 * Created by cuiduo on 2017/8/14.
 */
...
//    private static final String UNREACHABLE_PING = "100%";
    ...
    private String parseIpFromPing(String ping) {
        String ip = "";
        if (ping.contains(UtilClass.getFROM_PING())) {
            // Get ip when ttl exceeded
            int index = ping.indexOf(UtilClass.getFROM_PING());

            ip = ping.substring(index + 5);
            if (ip.contains(UtilClass.getPARENTHESE_OPEN_PING())) {
                // Get ip when in parenthese
                int indexOpen = ip.indexOf(UtilClass.getPARENTHESE_OPEN_PING());
                int indexClose = ip.indexOf(UtilClass.getPARENTHESE_CLOSE_PING());
            ...
            int indexOpen = ping.indexOf(UtilClass.getPARENTHESE_OPEN_PING());
            int indexClose = ping.indexOf(UtilClass.getPARENTHESE_CLOSE_PING());
    ...
    private String parseIpToPingFromPing(String ping) {
        String ip = "";
        if (ping.contains(UtilClass.getPING())) {
            // Get ip when ping succeeded
            int indexOpen = ping.indexOf(UtilClass.getPARENTHESE_OPEN_PING());
            int indexClose = ping.indexOf(UtilClass.getPARENTHESE_CLOSE_PING());
    ...
    private String parseTimeFromPing(String ping) {
        String time = "";
        if (ping.contains(UtilClass.getTIME_PING())) {
            int index = ping.indexOf(UtilClass.getTIME_PING());
    ...
    private class ExecutePingAsyncTask extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            if (hasConnectivity()) {
                try {
                    ...
                    Log.d(TAG, "resString====" + res);
                    TracerouteContainer trace;
                    String ip = parseIpFromPing(res);
                    Log.d(TAG, "ipString===" + ip);
                    if (res.contains(UtilClass.getUNREACHABLE_PING()) && !res.contains(UtilClass.getEXCEED_PING())) {
        ...
        private String launchPing(String url) throws Exception {
            ...
            while ((s = stdInput.readLine()) != null) {
                res += s + "\n";
                if (s.contains(UtilClass.getFROM_PING()) || s.contains(UtilClass.getSMALL_FROM_PING())) {
                ...
                try {
                    if (!"".equals(result)) {
                        if (context.getString(R.string.no_connectivity).equals(result)) {
                            ...
                            Log.d(TAG, "latestTrace===" + latestTrace);
                            Log.d(TAG, "latestTrace===" + latestTrace);
